...
let onScreenAlertAdd = (selectedItem) => {
    ...
    let find = menShopItemsData.find((y)=> y.id === selectedItem.id );      // this function finds the selectedItem in the menShopItemsData array so it can access its name property to put into alert. (tried to pass in item.name in generateMenShop but didnt work for some reason idk )
    let name = find.name;
